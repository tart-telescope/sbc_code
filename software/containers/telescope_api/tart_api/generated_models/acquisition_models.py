# DO NOT EDIT! This file is auto-generated from JSON schemas.
# To make changes, edit the schema files and regenerate.

# generated by datamodel-codegen:
#   filename:  acquisition.json
#   timestamp: 2026-03-18T06:18:00+00:00
#   version:   0.55.0

from __future__ import annotations
from typing import Annotated, Any
from pydantic import AwareDatetime, BaseModel, ConfigDict, Field, RootModel
from enum import IntEnum, StrEnum


class Model(RootModel[Any]):
    root: Any


class BinaryFlag(IntEnum):
    integer_0 = 0
    integer_1 = 1


class EmptyResponse(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )


class ErrorResponse(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    msg: str


class Message(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    description: str
    error: str
    status_code: int


class JWTErrorResponse(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    message: Message


class JWTAuthHeader(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    authorization: Annotated[
        str,
        Field(
            alias="Authorization",
            pattern="^JWT [A-Za-z0-9-_]+\\.[A-Za-z0-9-_]+\\.[A-Za-z0-9-_]+$",
        ),
    ]


class ChannelIndex(RootModel[int]):
    root: Annotated[int, Field(ge=0, le=23)]


class TelescopeMode(StrEnum):
    off = "off"
    diag = "diag"
    raw = "raw"
    vis = "vis"
    vis_save = "vis_save"
    cal = "cal"
    rt_syn_img = "rt_syn_img"


class LoopMode(StrEnum):
    loop = "loop"
    single = "single"
    loop_n = "loop_n"


class LoopCount(RootModel[int]):
    root: Annotated[int, Field(ge=0, le=100)]


class SampleExponent(RootModel[int]):
    root: Annotated[int, Field(ge=16, le=24)]


class FPGABinaryFlag(IntEnum):
    integer_0 = 0
    integer_1 = 1


class FPGAStateFlag(RootModel[int]):
    root: Annotated[int, Field(ge=0, le=7)]


class UTCTimestamp(RootModel[AwareDatetime]):
    root: AwareDatetime


class Hostname(RootModel[str]):
    root: str


class Frequency(RootModel[float]):
    root: Annotated[float, Field(ge=0.0)]


class Coordinate(RootModel[float]):
    root: float


class Altitude(RootModel[float]):
    root: Annotated[float, Field(ge=0.0, le=8848.0)]
    """
    Altitude in meters above sea level (0 to 8848m - Mt. Everest height)
    """


class AntennaPosition(RootModel[list[float]]):
    root: Annotated[list[float], Field(max_length=3, min_length=3)]


class AntennaPositions(RootModel[list[AntennaPosition]]):
    root: list[AntennaPosition]


class Filename(RootModel[str]):
    root: str


class Checksum(RootModel[str]):
    root: str


class SampleExponentResponse(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    n_samples_exp: Annotated[SampleExponent, Field(alias="N_samples_exp")]
    """
    Exponent for number of samples (2^exp)
    """


class SyncResponse(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    sync: BinaryFlag
    """
    Sync flag: 0=disabled (acquire immediately), 1=enabled (wait for next allowed second before acquiring)
    """


class SyncAcquireAtSecond(RootModel[int]):
    root: Annotated[int, Field(ge=0, le=59)]


class SyncAcquireAtSecondsResponse(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    sync_acquire_at_seconds: list[SyncAcquireAtSecond]
    """
    Allowed seconds-of-the-minute (0-59) at which acquisition may start. When sync is enabled, the system waits until the clock reaches the next second in this list before beginning acquisition.
    """


class SaveFlagResponse(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    save: BinaryFlag
    """
    Save flag: 0=disabled, 1=enabled
    """
